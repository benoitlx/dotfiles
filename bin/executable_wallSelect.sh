...
# Get focused monitor
# TODO replace by `niri msg --json focused-output`
...
monitor_width=$(niri msg --json focused-output | jq '.logical.width')
scale_factor=$(niri msg --json focused-output | jq '.logical.scale')
...
# Run matugen script
sleep 0.5
[[ -n "$wall_selection" ]] && "matugenMagick.sh" --dark
